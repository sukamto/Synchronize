 ...
 * @author z0006cpz
 */
public class CM_CSV extends FileMethods {
    ...
    private boolean grabStructures() throws Exception {
    ...
    private void addMissingColumns() throws IOException {
        ...
        int fileColumnCount = getFileColumnCount(fList);
        setWriterName(csvFile);
        for (int i = 1; i < fList.size(); i++) {
            fList.set(i, fList.get(i) + "|");
            String[] split = fList.get(i).split(DELIMITER);
            int size = split.length;
            if (size > maxColumnCount) {
                maxColumnCount = size;
            }
            fList.set(i, fList.get(i).substring(0, fList.get(i).length() - 1));
            for (int j = size; j < fileColumnCount; j++) {
                fList.set(i, fList.get(i) + DELIMITER);
            }
        }
        int i = 1;
        for (int j = fileColumnCount; j < maxColumnCount; j++) {
            fList.set(0, fList.get(0) + DELIMITER + "E" + i);
            i++;
        }
        listToFile(fList);
    }

    private int getFileColumnCount(List<String> fList) {
        String head = fList.get(0) + "|";
    ...
    private boolean getTableStructure() throws SQLException {
        ...
        if (tableColumns.isEmpty()) {
            System.err.println(getCurrentDateTime() + "->Error grabbing table structure: Table " + destinationTable + " not found.");
            return false;
        }
        return true;
    }
    ...
    private void getCSVstructure() throws IOException {
        ...
        for (String s : split) {
            ...
            if (s.equalsIgnoreCase("skip")
                    ...
                    || s.toLowerCase().equalsIgnoreCase("bts_dn")
                    || s.toLowerCase().equalsIgnoreCase("dn")
                    || s.toLowerCase().equalsIgnoreCase("Alarm Time")
                    || s.toLowerCase().equalsIgnoreCase("AMR_RADIO_LINK_TIMEOUT")
